...
def list_pan(matid, tpp=None):
    ...
    for i in idPan:
        ...
        if pn.form(i) == 0:
            ...
            dic["band_x04"] = int(0 < be["ThickBand"] < 1) + int(
                0 < bd["ThickBand"] < 1
            )
            dic["band_y04"] = int(0 < bb["ThickBand"] < 1) + int(
                0 < bc["ThickBand"] < 1
            )
            dic["band_x2"] = int(0 < be["ThickBand"] >= 1) + int(
                0 < bd["ThickBand"] >= 1
            )
            dic["band_y2"] = int(0 < bb["ThickBand"] >= 1) + int(
                0 < bc["ThickBand"] >= 1
            )
...
def rep_pan(name, mat, tpp=None):
    ...
    for imat in mat:
        ...
        row = draw_table(
            xl, 1, "Виноградов А.Г. 8920-206-83-98", imat["Name"], "В цвет", formarsheet
        )
        rw1 = row
        index = 1
        for l in list_pan(imat["ID"], tpp):
            ...
            xl.mergecells(
                row - 1, 10, 1, hg=2, halign="c", valign="c", wrap="f", nf=fnum
            )
...
def draw_table(xl, rw1, customer, mat, bands, fs):
    ...
    head = [
        ...
        "Квадратура деталей",
        "=SUM(R[4]C:R[1000]C)",
        "Кол-во листов формата {0}".format(str(fs[0]) + "x" + str(fs[1])),
        "=RC[-2]*1.2/{0}".format(s_sheet),
    ]
    head2 = [
        "",
        "Кол-во сторон кромки",
        "",
        "",
        "",
        "",
    ...
    xl.txtformat(
        ...
        valign="c",
        wrap="t",
        ort=[0, 0, 0, 0, 0, 0, 90, 90, 0, 0, 0, 0],
        fsz=[11],
        nf=[fnum],
    )
    xl.txtformat(rw1 + 1, 10, halign="c", nf=[fnum])
    xl.txtformat(
        rw1 + 2, 1, halign="cccccccclclc", valign="c", nf=[fnum], bold="fftfftfft"
    )
    xl.gridset(tc=2)
    xl.grid(rw1, 1, 12, 3, "lrudvh")
    rw1 += 4
