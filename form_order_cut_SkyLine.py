...
def draw_table(xl, rw1, customer, mat, bands, fs):
    ...
    head = ['№', 'Длина','', 'x', 'Ширина','', 'кол-во', 'текстура', 'Квадратура деталей', \
            '=SUM(R[4]C:R[1000]C)', 'Кол-во листов формата {0}'.format(str(fs[0])+'x'+str(fs[1])), '=RC[-2]*1.2/{0}'.format(s_sheet)]
    head2 = ['','Кол-во сторон кромки','','','','','','','Кол-во кромки 0,4мм','=SUM(C[-1])/10^3','Кол-во кромки 2мм','=SUM(C[-1])/10^3']
    head3 = ['','0,4мм','2мм','x','0,4мм','2мм','','','Купить кромки 0,4мм','=R[-1]C*1.1','Купить кромки 2мм','=R[-1]C*1.1']
    ...
    xl.txtformat(rw1, 1, halign='c', valign='c', wrap='t', ort=[0,0,0,0,0,0,90,90,0,0,0,0], fsz=[11], nf=[fnum])
