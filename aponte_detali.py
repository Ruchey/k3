...
LISTKEY = ('num', 'name', 'length', 'width', 'band_x04', 'band_y04', 'band_x2', 'band_y2', 'chamfer', \
           'slots_x_len', 'slots_y_len', 'holes', 'cnt_hings_x', 'cnt_hings_y', 'gluing', 'color_band')
...
def list_pan(matid, tpp=None):
    ...
    for i in idPan:
        ...
        if pn.form(i) == 0:
            ...
            dic['length'] += fuga * (dic['band_y04'] + dic['band_y2'])  # добавляем к размеру прифуговку
            dic['width'] += fuga * (dic['band_x04'] + dic['band_x2'])  # добавляем к размеру прифуговку
            dic['chamfer'] = pn.cnt_chamfer_pan(i)
            dic['slots_x_len'] = pn.slots_x_len(i)  # Суммарная длина пропилов вдоль панели
            dic['slots_y_len'] = pn.slots_y_len(i)  # Суммарная длина пропилов поперёк панели
            ...
            if (45 < pdir <= 135) or (225 < pdir <= 315):
                ...
                dic['slots_x_len'], dic['slots_y_len'] = dic['slots_y_len'], dic['slots_x_len']
                dic['cnt_hings_x'], dic['cnt_hings_y'] = dic['cnt_hings_y'], dic['cnt_hings_x']
            listPan.append(dic)
...
def rep_pan(name, mat, tpp=None):
    ...
    for imat in mat:
        for l in list_pan(imat['ID'], tpp):
            val = (l['name'], l['num'], imat['Thickness'], l['length'], l['width'], l['cnt'], l['band_x04'], \
                   l['band_y04'], l['band_x2'], l['band_y2'], '', '', '', l['chamfer'], '', '', \
                   l['slots_x_len'], l['slots_y_len'], l['cnt_hings_x'], l['cnt_hings_y'], l['holes'], l['gluing'],
                   l['color_band'], imat['Name'])
            xl.putval(row, 1, val)
...
def draw_table(rw1, rw2):
    'Форматируем таблицу'
    head = ['Наименование', '№ п/п', 'Толщина', 'Высота,  мм', 'Ширина, мм', 'шт', '0,4 мм', '', '2  мм', '', 'Отметка',
            '04 кр', '2 кр', 'Угол, шт', 'Радиус', 'Фрезер', 'Паз, п/м', '', 'Петли, шт', '', 'Присадка, шт', 'Склейка',
            'Цвет кромки', 'Материал']
    sp_tab = [19, 2.4, 6.7, 8, 8, 6.6, 2, 2, 2, 2, 1.6, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 7, 25]
    ...
    xl.txtformat(rw1, 1, halign='c', valign='c', wrap='t', \
                 ort=[0, 0, 90, 0, 0, 0, 0, 0, 0, 0, 90, 90, 90, 90, 90, 90, 0, 0, 0, 0, 90, 90, 0, 0], \
                 fsz=[12, 8, 8, 12, 12, 12, 12, 12, 12, 12, 8, 12, 12, 12, 12, 12, 12, 12, 12, 12, 8, 8, 12, 12])
    gh = rw2 - rw1
    xl.gridset(tc=2)
