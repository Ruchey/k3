...
def detali(namesheet, data=None, tabcolor=1):
    ...
    if object:
        ...
        val = '№' + namesheet.replace('Изд.', '') + ' ' + name + (' арт. ' + article if article else '')
    ...
    if matid:
        ...
        for i in matid:
            ...
            val = [j, name, '', '', prop['UnitsName'], nm.matcount(i, object)/objcnt, price, '=RC[-2]*RC[-1]']
            xl.putval(rw, 1, val)
            xl.txtformat(rw, 1, 'llccrrrr', 'c', 'f', [fgen, txt, txt, txt, txt, fnum, frub, frub])
            if not xldic:
                xlDic[prop['Name']] = [("'" + namesheet + "'" + "!" + xl.num2col(2) + str(rw)), ("'" + namesheet + "'" + "!" + xl.num2col(7) + str(rw))]
            rw += 1
        xl.gridset(tc=5)
    ...
    if acc:
        ...
        for i in acc:
            ...
            xl.putval(rw, 1, val)
            xl.txtformat(rw, 1, 'llccrrrr', 'c', 'f', [fgen, txt, txt, txt, txt, '', frub, frub])
            if not xldic:
                xlDic[name] = [("'" + namesheet + "'" + "!" + xl.num2col(2) + str(rw)), ("'" + namesheet + "'" + "!" + xl.num2col(7) + str(rw))]
            rw += 1
        xl.gridset(tc=5)
        xl.grid(rwgrid, 1, len(val), rw-rwgrid, 'lrudvh')
    ...
    if accl:
        ...
        for i in accl:
            ...
            val = [j, name, i[3], i[4]/objcnt, 'шт', i[5], price, '=RC[-4]*RC[-2]*RC[-1]']
            xl.putval(rw, 1, val)
            xl.txtformat(rw, 1, 'llccrrrr', 'c', 'f', [fgen, txt, txt, fnum, txt, '', frub, frub])
            if not xldic:
                xlDic[name] = [("'" + namesheet + "'" + "!" + xl.num2col(2) + str(rw)), ("'" + namesheet + "'" + "!" + xl.num2col(7) + str(rw))]
            rw += 1
        xl.gridset(tc=5)
    ...
    if prof:
        ...
        for i in prof:
            ...
            name1 = prop['Name'] + (' арт.' + prop['Article'] if prop['Article'] else '')
            name = (name1 if not xldic else '=' + xldic[name1][0])
            price = (eval(prop['Price']) if not xldic else '=' + xldic[name1][1])
            val = [j, name, '', '', prop['UnitsName'], (i[1]/10**3)/objcnt, price, '=RC[-2]*RC[-1]']
            xl.putval(rw, 1, val)
            xl.txtformat(rw, 1, 'llccrrrr', 'c', 'f', [fgen, txt, txt, txt, txt, fnum, frub, frub])
            if not xldic:
                xlDic[name] = [("'" + namesheet + "'" + "!" + xl.num2col(2) + str(rw)), ("'" + namesheet + "'" + "!" + xl.num2col(7) + str(rw))]
    ...
    if longs:
        ...
        for i in longs:
            ...
            name1 = name0 + prop['Name'] + (' арт.' + prop['Article'] if prop['Article'] else '')
            name = (name1 if not xldic else '=' + xldic[name1][0])
            price = (eval(prop['Price']) if not xldic else '=' + xldic[name1][1])
    ...
    if bands:
        ...
        for i in bands:
            ...
            name = (name1 if not xldic else '=' + xldic[name1][0])
            price = (eval(prop['Price']) if not xldic else '=' + xldic[name1][1])
            val = [j, name, '', i[1], (i[0]/10**3)/objcnt, eval(prop.get('WasteCoeff','1')), price, '=RC[-3]*RC[-2]*RC[-1]']
            xl.putval(rw, 1, val)
            xl.txtformat(rw, 1, 'llccrrrr', 'c', 'f', [fgen, txt, txt, '', fnum, fnum, frub, frub])
            if not xldic:
                xlDic[name] = [("'" + namesheet + "'" + "!" + xl.num2col(2) + str(rw)), ("'" + namesheet + "'" + "!" + xl.num2col(7) + str(rw))]
            rw += 1
        xl.gridset(tc=5)
...
def kompred(xlDic, tabcolor=1):
    ...
    try:
        ...
        pass
    head = "Коммерческое предложение от {}".format(datetime.date.today().strftime('%d.%m.%Y'))
    xl.header(rw,1,head, tc=1, tas=0)
    xl.mergecells(rw, 1, 7, 1)
    xl.gridset(wt='xlMedium', tc=5)
    xl.grid(1, 1, 7, 2, 'lrudh')
    rw += 2
    head = ['№ п/п', 'Изделие', 'Техническое описание', 'Цена', 'Кол-во', 'Стоимость', 'Эскиз', 'С-сть', 3]
    ...
    for i in lstobj:
        ...
        cnt = elems['Count']
        cost = '=' + prlst[up]
        tech_over = '="' + "Размер (ШхГхВ) " + str(round(elems['XUnit'])) + "x" + str(round(elems['YUnit'])) + "x" + str(round(elems['ZUnit'])) + " мм" + '; "'
        ...
        if prof:
            ...
            for i in prof:
                prop = nm.properties(i[0])
                if prop.get('NoKP')!=1:
                    title = prop['Name'] + (' арт.' + prop['Article'] if prop['Article'] else '')
                    adr = xlDic[title][0]
                    tech_over += '&' + 'IF(OFFSET({0},0,-1)>0,{0}&"; ","")'.format(adr)
        ...
        if longs:
            ...
            for i in longs:
                ...
                tng = bs.tngoods(i[3])[0]
                name0 = tng['GroupName'] + ' ' + tng['Name'] + ' '
                title = name0 + prop['Name'] + (' арт.' + prop['Article'] if prop['Article'] else '')
                adr = xlDic[title][0]
                tech_over += '&' + 'IF(OFFSET({0},0,-1)>0,{0}&"; ","")'.format(adr)
        ...
        xl.putval(rw, 1, val)
        picxy[up] = [rw, 7]
        xl.txtformat(rw, 1, 'rllrrrrrr', 'cttc', 'ft', [fgen, txt, txt, frub, fgen, frub, fgen, frub, fgen])
